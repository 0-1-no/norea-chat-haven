        ...
        <Card className="border-none shadow-sm bg-white">
          <CardContent className="p-6">
            ...
            <div className="flex gap-2 mb-6">
              <Button variant={selectedTaskGroup === 'today' ? 'default' : 'outline'} size="sm" className="flex items-center gap-1" onClick={() => setSelectedTaskGroup('today')}>
                <Clock size={16} />
                <span>I dag</span>
              </Button>
              <Button variant={selectedTaskGroup === 'upcoming' ? 'default' : 'outline'} size="sm" className="flex items-center gap-1" onClick={() => setSelectedTaskGroup('upcoming')}>
                <ListTodo size={16} />
                <span>Snart</span>
              </Button>
            ...
            <div className="space-y-2 mb-6">
              {getTasksToShow().map(task => <div key={task.id} className="flex items-center gap-3 p-3 hover:bg-gray-50 rounded-md transition-colors cursor-pointer" onClick={() => navigate('/assistant/todo/tasks')}>
                  <div className={`h-2 w-2 rounded-full ${task.completed ? 'bg-green-500' : task.priority === 'high' ? 'bg-red-500' : task.priority === 'medium' ? 'bg-amber-500' : 'bg-blue-500'}`} />
