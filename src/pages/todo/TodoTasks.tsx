...
import { ChevronDown, ChevronRight, Plus, Search, Filter, Calendar, Inbox, Clock, CheckCircle } from 'lucide-react';
...
const TodoTasks = () => {
  ...
  const taskGroups = [{
    ...
    tasks: [{
      ...
      subtasks: [{
        id: 401,
        title: "Samle alle dokumenter",
        completed: true
      }, {
        id: 402,
        title: "Dobbeltsjekk fradrag",
        completed: false
      }, {
        id: 403,
      ...
      subtasks: [{
        id: 501,
        title: "Lag gjesteliste",
        completed: true
      }, {
        id: 502,
        title: "Send invitasjoner",
        completed: true
      }, {
        id: 503,
        title: "Planlegg meny",
    ...
    tasks: [{
      ...
      subtasks: [{
        id: 601,
        title: "Bestemt destinasjon",
        completed: true
      }, {
        id: 602,
        title: "Sjekk hoteller",
        completed: false
      }, {
        id: 603,
        title: "Bestill fly",
  ...
  const getFilteredTaskGroups = () => {
    ...
    if (activeFilter === 'inbox') {
      ...
      const inboxTasks = taskGroups.filter(group => group.id !== 'completed').map(group => ({
        ...group,
  ...
  return <TodoLayout>
      <div className="space-y-6 max-w-3xl mx-auto">
        <div className="flex flex-col sm:flex-row justify-between gap-4">
          <div className="relative flex-1">
            ...
            <Input placeholder="Søk i oppgaver..." className="pl-9" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} />
          </div>
        ...
        <div className="space-y-4">
          {filteredTaskGroups.length > 0 ? filteredTaskGroups.map(group => <Card key={group.id} className="shadow-sm">
                <Collapsible open={expandedSections.includes(group.id)} onOpenChange={() => toggleSectionExpanded(group.id)}>
                  <div className="flex items-center py-3 px-4">
                    <CollapsibleTrigger asChild>
                      <Button variant="ghost" size="sm" className="h-6 w-6 p-0 mr-2">
                        {expandedSections.includes(group.id) ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}
                      </Button>
                    </CollapsibleTrigger>
                  ...
                  <CollapsibleContent>
                    <CardContent className="pt-3 pb-1">
                      <div className="space-y-0">
                        {group.tasks.map(task => <TaskItem key={task.id} task={task} onToggleComplete={toggleTaskCompletion} onOpenDetail={openTaskDetail} />)}
                      </div>
      ...
      <TaskDetailModal task={selectedTask} open={isDetailOpen} onClose={() => setIsDetailOpen(false)} onSave={saveTaskChanges} />
    </TodoLayout>;
};
